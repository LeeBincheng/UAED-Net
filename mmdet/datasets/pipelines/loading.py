...
class LoadAugmentedImageFromFile:
    ...
    Args:
        with_aug (bool): 是否加载增强图像
        aug_root (str): 增强图像的根目录
        to_float32 (bool): 是否将图像转换为float32类型
        color_type (str): 图像的颜色类型
    """
    ...
    def __call__(self, results):
        ...
        if self.with_aug and self.aug_root:
            ...
            if 'img_info' in results:
                ...
                results['aug_img_prefix'] = self.aug_root
                aug_file = osp.join(self.aug_root, filename)

                # 检查文件是否存在
                try:
                # if osp.exists(aug_file):
                #     # 加载增强图像
                #     aug_img = mmcv.imread(aug_file, self.color_type)
                #
                #     # 调整增强图像尺寸以匹配原图
                #     if 'img_shape' in results:
                #         h, w = results['img_shape'][:2]
                #         aug_img = mmcv.imresize(aug_img, (w, h))
                    if self.file_client.exists(aug_file):
                        # 使用与原始图像相同的加载方式
                        aug_bytes = self.file_client.get(aug_file)
                        aug_img = mmcv.imfrombytes(aug_bytes, flag=self.color_type)
                        ...
                        warnings.warn(f"Aug image not found: {aug_file}")
                        results['aug_img'] = None

                except Exception as e:
                    warnings.warn(f'Error loading aug file {aug_file}: {str(e)}')
                    results['aug_img'] = None
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(with_aug={self.with_aug}, '
        repr_str += f'aug_root={self.aug_root}, '
